...
# NOTE: Build and push image before applying this resource:
#   $ACR = terraform output -raw acr_login_server
#   az acr build --registry ($ACR -split '\.')[0] `
#     --image cloudkitchen:latest `
#     https://github.com/flye238/cloudkitchen.git
resource "azurerm_container_app" "main" {
  ...
  template {
    ...
    container {
      ...
      env {
        name  = "POSTGRES_DB"
        value = azurerm_postgresql_flexible_server_database.kitchendb.name
      }
      env {
        name  = "POSTGRES_USER"
